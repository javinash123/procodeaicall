...
import { Plus, Trash2, Loader2, List, Edit2, Search, ChevronLeft, ChevronRight, LayoutDashboard, Users, CreditCard as CreditCardIcon, UserCog, LogOut, User, Bot } from "lucide-react";
import { 
  Table, 
  TableBody, 
  TableCell, 
...
export default function AdminPlans() {
  ...
  const adminNavItems = [
    ...
    { id: "admin-users", label: "User Management", icon: Users, path: "/admin/dashboard?tab=admin-users" },
    { id: "plans", label: "Plans", icon: CreditCardIcon, path: "/admin/plans" },
    { id: "profile", label: "Profile", icon: UserCog, path: "/admin/dashboard?tab=profile" },
  ];
    ...
    <div className="w-64 border-r bg-card flex flex-col h-full overflow-hidden shrink-0">
      <div className="p-6 border-b">
        <div className="flex items-center gap-2 mb-2">
          ...
          <span className="font-bold text-xl tracking-tight text-foreground">AI Agent</span>
        </div>
      </div>
      <ScrollArea className="flex-1 py-4">
        <div className="px-3 space-y-1">
          {adminNavItems.map((item) => (
            <Button
              key={item.id}
              variant={item.id === "plans" ? "secondary" : "ghost"}
              className="w-full justify-start gap-3 h-10 px-3 hover-elevate"
              onClick={() => setLocation(item.path)}
            >
              <item.icon className={`w-4 h-4 ${item.id === "plans" ? "text-primary" : "text-muted-foreground"}`} />
      ...
      <div className="p-4 border-t mt-auto">
        <DropdownMenu>
          ...
          <DropdownMenuContent align="end" className="w-56">
            ...
            <DropdownMenuItem onClick={() => setLocation("/admin/dashboard?tab=profile")}>
